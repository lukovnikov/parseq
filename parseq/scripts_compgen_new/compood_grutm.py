...
class HybridSeqDecoder(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor,
        ...
        for decoder in self.alldecoders:
            ret, pred_trees = self.innerforward(x, gold=gold, decoder=decoder)
            rets.append(ret)
            pred_treeses.append(pred_trees)

        # merge
        newret = {}
        for i, (k, v) in enumerate(rets[0].items()):
            newret[f"{k}_{i}"] = v
            if k not in newret:
                newret[k] = []
            newret[k].append(v)
    ...
    def innerforward(self, x:torch.Tensor,
                     gold: torch.Tensor=None,
                     decoder=None):
        preds, prednll, maxmaxnll, entropy, total, avgconf, sumnll, stepsused = decoder.get_prediction(x)
        ...
        gold_trees = tensor_to_trees(gold, vocab=decoder.vocab)
        pred_trees = tensor_to_trees(preds, vocab=decoder.vocab)
