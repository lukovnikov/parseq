...
class SeqDecoderBaseline(torch.nn.Module):
    ...
    def train_forward(self, x:torch.Tensor, y:torch.Tensor, mask=None):  # --> implement one step training of cell
        ...
        loss, acc, elemacc = self.compute_loss(logits, tgt, mask=newmask)
        return {"loss": loss, "acc": acc, "elemacc": elemacc}, logits

    def extract_training_example(self, x, y, supmask=None):
        ymask = (y != 0).float()
        ...
        newmask = None
        if supmask is not None:
            newmask = supmask
            newmask = torch.cat([newmask, torch.zeros_like(newmask[:, 0:1])], 1)
        # append EOS
...
class SeqDecoderAE(torch.nn.Module):
    ...
    def train_forward(self, x:torch.Tensor, y:torch.Tensor,
        ...
        # extract a training example from y:
        blank_ret = {"loss": torch.tensor([0.]).to(x.device),
                     "acc": torch.tensor([-1.]).to(x.device),
                     "elemacc": torch.tensor([-1.]).to(x.device)}    # TODO: fix
