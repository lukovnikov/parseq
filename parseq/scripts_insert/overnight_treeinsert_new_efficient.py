...
class TransformerTagger(TreeInsertionTagger):
    ...
    def __init__(self, dim, vocab:Vocab=None, numlayers:int=6, numheads:int=6,
        ...
        self.structids = [torch.tensor(self.vocab[e]) for e in self.structtokens]
        self.slotids = [torch.tensor(self.vocab[e]) for e in self.slottokens]
        self.nonessentialids = self.structids + self.slotids

        # self.slotrelposemb = torch.nn.Embedding(5, config.d_model)
        self.slotlin = torch.nn.Linear(config.d_model, config.d_model * 5)
        # self.anc_lin = torch.nn.Linear(config.d_model, config.d_model)
        # self.desc_lin = torch.nn.Linear(config.d_model, config.d_model)
        # self.parent_lin = torch.nn.Linear(config.d_model, config.d_model)
        # self.left_lin = torch.nn.Linear(config.d_model, config.d_model)
        # self.right_lin = torch.nn.Linear(config.d_model, config.d_model)
    ...
    def get_backbone(self, tokens, attnmask, relpos):
        backbone_positions = torch.zeros_like(tokens)
        backbone_revpositions = torch.zeros_like(tokens) - 1
        anc_positions = torch.zeros_like(tokens) - 1
        desc_positions = torch.zeros_like(tokens) - 1
        ...
        for i in range(len(tokens)):
            ...
            for j in range(len(tokens[i])):
                token = tokens[i, j]
                if token in self.structids:
                    pass
                elif token in self.slotids:
                    # if token == self.vocab["@^"]:       # ancestor
                    #     anc_positions[i, j] = k + 1
                    # elif token == self.vocab["@v"]:     # descendant
                    #     desc_positions[i, j] = k
                    # elif token == self.vocab["@-"]:     # sibling
                    #     pass
                    pass
                elif token == self.vocab[self.vocab.padtoken]:        # assumes pad tokens are only at the end
                    maxlen = max(maxlen, k)
        ...
        backbone_relpos = relpos.gather(1, backbone_positions[:, :, None, None].expand(-1, -1, relpos.size(2), relpos.size(3)))
        backbone_relpos = backbone_relpos.gather(2, backbone_positions[:, None, :, None].expand(-1, backbone_relpos.size(1), -1, backbone_relpos.size(3)))

        # process relpos
        ancrelposmask = relpos[:, :, :, 3] == self.rel_pos.D["ancslot"]
        descrelposmask = relpos[:, :, :, 3] == self.rel_pos.D["descslot"]
        parentrelposmask = relpos[:, :, :, 3] == self.rel_pos.D["parent"]
        leftrelposmask = relpos[:, :, :, 3] == self.rel_pos.D["left"]
        rightrelposmask = relpos[:, :, :, 3] == self.rel_pos.D["right"]
        assert torch.all(ancrelposmask.sum(2) <= 1)
        assert torch.all(descrelposmask.sum(2) <= 1)
        assert torch.all(parentrelposmask.sum(2) <= 1)
        assert torch.all(leftrelposmask.sum(2) <= 1)
        assert torch.all(rightrelposmask.sum(2) <= 1)
        for i in range(len(relpos)):
            nz = ancrelposmask[i].nonzero()
            if len(nz) > 0:
                anc_positions[i].scatter_(0, nz[:, 0], backbone_revpositions[i][nz[:, 1]])
            nz = descrelposmask[i].nonzero()
            if len(nz) > 0:
                desc_positions[i].scatter_(0, nz[:, 0], backbone_revpositions[i][nz[:, 1]])
            nz = parentrelposmask[i].nonzero()
            if len(nz) > 0:
                parent_positions[i].scatter_(0, nz[:, 0], backbone_revpositions[i][nz[:, 1]])
            nz = leftrelposmask[i].nonzero()
            if len(nz) > 0:
                left_positions[i].scatter_(0, nz[:, 0], backbone_revpositions[i][nz[:, 1]])
            nz = rightrelposmask[i].nonzero()
            if len(nz) > 0:
                right_positions[i].scatter_(0, nz[:, 0], backbone_revpositions[i][nz[:, 1]])

        return backbone_tokens, backbone_attnmask, backbone_relpos, (anc_positions, desc_positions, parent_positions, left_positions, right_positions)

    def forward(self, tokens:torch.Tensor=None, enc=None, encmask=None, cache=None, relpos=None, attnmask=None):
        _tokens, _attnmask, _relpos, (anc_positions, desc_positions, parent_positions, left_positions, right_positions)\
            = self.get_backbone(tokens, attnmask, relpos)
        if _attnmask is not None:
            # compress inputs
            attnmask_q = _attnmask.sum(1) > 0
            attnmask_k = _attnmask.sum(2) > 0
            compress_mask = attnmask_q | attnmask_k
        _attnmask = (_tokens != 0) if _attnmask is None else _attnmask
        # if not self.baseline:
        #     padmask = padmask[:, 1:]
        embs = self.emb(_tokens)
        if self.abs_pos is not None and self.abs_pos is not False:
            posembs = self.abs_pos(torch.arange(_tokens.size(1), device=_tokens.device))[None]
            embs = embs + posembs
        use_cache = False
        if cache is not None:
            embs = embs[:, -1:, :]

        _ret = self.decoder(inputs_embeds=embs, attention_mask=_attnmask,
        ...
        slot_cs = self.slotlin(c)       # (bs, compressed_seqlen, 5*dim)
        anc_c, desc_c, parent_c, left_c, right_c = slot_cs.chunk(5, -1)
        # decompress
        anc_c = anc_c.gather(1, anc_positions.clamp_min(0)[:, :, None].expand(-1, -1, anc_c.size(2)))
        desc_c = desc_c.gather(1, desc_positions.clamp_min(0)[:, :, None].expand(-1, -1, desc_c.size(2)))
        parent_c = parent_c.gather(1, parent_positions.clamp_min(0)[:, :, None].expand(-1, -1, parent_c.size(2)))
        left_c = left_c.gather(1, left_positions.clamp_min(0)[:, :, None].expand(-1, -1, left_c.size(2)))
        right_c = right_c.gather(1, right_positions.clamp_min(0)[:, :, None].expand(-1, -1, right_c.size(2)))
        # apply mask
        anc_c = anc_c * (anc_positions >= 0).float()[:, :, None]
        desc_c = desc_c * (desc_positions >= 0).float()[:, :, None]
        parent_c = parent_c * (parent_positions >= 0).float()[:, :, None]
        left_c = left_c * (left_positions >= 0).float()[:, :, None]
        right_c = right_c * (right_positions >= 0).float()[:, :, None]
        c = anc_c + desc_c + parent_c + left_c + right_c
