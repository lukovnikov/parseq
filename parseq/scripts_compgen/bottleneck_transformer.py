...
class VIBTransformerAttention(nn.Module):
    def __init__(self, config: TransformerConfig, rel_emb=None):
        super().__init__()
        self.config = config
        self.is_decoder = config.is_decoder
        ...
        self.vib_gate_k = nn.Linear(self.inner_dim, self.inner_dim, bias=False)
        self.vib_mu_k = nn.Linear(self.inner_dim, self.inner_dim, bias=True)
        self.vib_logvar_k = nn.Linear(self.inner_dim, self.inner_dim, bias=True)
        self.vib_ln_k = nn.LayerNorm(self.inner_dim, eps=self.config.layer_norm_epsilon)
        self.vib_gate_v = nn.Linear(self.inner_dim, self.inner_dim, bias=False)
        self.vib_mu_v = nn.Linear(self.inner_dim, self.inner_dim, bias=True)
        self.vib_logvar_v = nn.Linear(self.inner_dim, self.inner_dim, bias=True)
        self.vib_ln_v = nn.LayerNorm(self.inner_dim, eps=self.config.layer_norm_epsilon)
    ...
    def reset_parameters(self):
        ...
        self.vib_gate_k.weight.data.normal_(mean=0.0, std=FACTOR * ((n_heads * d_kv) ** -0.5))
        self.vib_mu_k.weight.data.normal_(mean=0.0, std=FACTOR * ((n_heads * d_kv) ** -0.5))
        self.vib_mu_k.bias.data.fill_(0)
        self.vib_logvar_k.weight.data.normal_(mean=0.0, std=FACTOR * ((n_heads * d_kv) ** -0.5))
        self.vib_logvar_k.bias.data.fill_(0)
        self.vib_gate_v.weight.data.normal_(mean=0.0, std=FACTOR * ((n_heads * d_kv) ** -0.5))
        self.vib_mu_v.weight.data.normal_(mean=0.0, std=FACTOR * ((n_heads * d_kv) ** -0.5))
        self.vib_mu_v.bias.data.fill_(0)
        self.vib_logvar_v.weight.data.normal_(mean=0.0, std=FACTOR * ((n_heads * d_kv) ** -0.5))
        self.vib_logvar_v.bias.data.fill_(0)
    ...
    def forward(
        ...
        # do vib attention
        # create a (bs, n_heads, qlen, klen, dim) matrix from vib
        m = q[:, :, :, None, :] + k[:, :, None, :, :]
        if relpos is not None:
            relpos_vectors_k = self.rel_emb.get_vectors(q, relpos, keyorvalue="k")
            m = m + relpos_vectors_k

        _m = m
        m = m.permute(0, 2, 3, 1, 4)
        m = m.view(m.size(0), m.size(1), m.size(2), -1)     # (bs, qlen, klen, n_heads * dim)

        # apply network
        m = torch.nn.functional.leaky_relu(m, negative_slope=0.1)
        m = m * torch.sigmoid(self.vib_gate_k(m))
        mu, logvar = self.vib_mu_k(m), self.vib_logvar_k(m)
        sample = mu + torch.exp(0.5 * logvar) * torch.randn_like(mu)
        sample = sample.view(m.size(0), m.size(1), m.size(2), _m.size(1), _m.size(4))
        sample = sample.permute(0, 3, 1, 2, 4)  # (bs, nh, qlen, klen, dim)

        # region build graph
        g = dgl.DGLGraph()
        # make nodes
        g.add_nodes(q.size(0)*q.size(1)+k.size(0)*k.size(1))
        _q = q.transpose(1, 2)      # (bs, qlen, n_heads, dim)
        _q = _q.view(-1, _q.size(2), _q.size(3))     # (bs * qlen, n_heads, dim)
        _k = k.transpose(1, 2)
        _k = _k.view(-1, _k.size(2), _k.size(3))     # (bs * klen, n_heads, dim)
        _v = v.transpose(1, 2)
        _v = _v.view(-1, _v.size(2), _v.size(3))     # (bs * klen, n_heads, dim)
        g.ndata["h"] = torch.cat([_q, _k], 0)
        g.ndata["v"] = torch.cat([torch.zeros_like(_q), _v], 0)
        kstart = len(_q)
        # make edges
        # edges from every non-masked key element to every query element within one example
        bs = k.size(0)
        klen = k.size(1)
        qlen = q.size(1)
        froms = torch.arange(klen, device=q.device)[None, :].repeat(1, qlen) + torch.arange(bs, device=q.device)[:, None] * klen
        # (bs, klen * qlen) --> [[1 2 3 1 2 3], [4 5 6 4 5 6], ...] for klen = 3, qlen=2
        froms = froms.view(-1) + kstart
        tos = torch.arange(qlen, device=q.device)[:, None].repeat(1, qlen).view(-1)
        # (bs, klen * qlen) --> [[0 0 0 1 1 1], [2 2 2 3 3 3], ...]
        g.add_edges()
        # endregion
        ...
        m = q[:, :, :, None, :] + v[:, :, None, :, :]
        if relpos is not None:
            relpos_vectors_v = self.rel_emb.get_vectors(q, relpos, keyorvalue="v")
            m = m + relpos_vectors_v
