...
def load_ds(domain="restaurants", min_freq=0, top_k=np.infty, nl_mode="bert-base-uncased", trainonvalid=False, fullsimplify=False, add_domain_start=True):
    ds = OvernightDatasetLoader(simplify_mode="light" if not fullsimplify else "full", simplify_blocks=True, restore_reverse=DATA_RESTORE_REVERSE)\
        .load(domain=domain, trainonvalid=trainonvalid)
...
def run(domain="restaurants",
    ...
    wandb.init(project=f"overnight_pretrain_bert",
