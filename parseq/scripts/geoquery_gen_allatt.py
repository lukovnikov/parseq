...
class BasicGenModel(TransitionModel):
    ...
    def forward(self, x:State):
        ...
        if "prevstates" not in mstate:
            ...
            _ctx_mask = torch.cat([ctx_mask, torch.ones(mstate.prevstates.size(0), mstate.prevstates.size(1), dtype=ctx_mask.dtype, device=ctx_mask.device)], 1)
            mstate.prevstates = torch.cat([mstate.prevstates, enc[:, None, :]], 1)
        ...
        if self.store_attn:
            ...
            atts = q.pad_tensors([x.stored_attentions, alphas.detach()[:, None, :]], 2, 0)
