...
class ONRLSTMTransition(TransitionModel):
    def __init__(self, indim, hdim, num_layers=1, dropout:float=0., p_step=0., p_min=1., **kw):
        super(ONRLSTMTransition, self).__init__(**kw)
        ...
        self.p_step, self.p_min = p_step, p_min
    ...
    def forward(self, x, state):
        ...
        for l in range(len(self.cells)):
            y = self.dropout(y)
            mask = torch.arange(cs[l].size(-1), device=cs[l].device, dtype=torch.float)
            mask = (mask[None, :] >= (perc[:, None] * cs[l].size(-1))).float()

            y, _c = self.cells[l](y, (hs[l], cs[l]))

            cs[l] = (1 - mask) * cs[l] + mask * _c
            hs[l] = y
...
def try_rr_lstm():
    t = ONRLSTMTransition(5, 4, 2, dropout=0.1, p_step=0.3, p_min=0.3)
    x = torch.rand(3, 5)
...
class BasicGenModel(TransitionModel):
    ...
    def __init__(self, embdim, hdim, numlayers:int=1, dropout=0.,
        ...
        decoder_rnn = ONRLSTMTransition(dec_rnn_in_dim, hdim, numlayers, dropout=dropout,
    ...
    def forward(self, x:State):
        ...
        for i in range(len(x)):
            if (x.prev_actions[i] == self.open_id).item() is True:       # opening token "("
                mstate.rnnstate.levels[i] += 1
            elif (x.prev_actions[i] == self.close_id).item() is True:    # closing token ")"
                mstate.rnnstate.levels[i] = (mstate.rnnstate.levels[i] - 1).clamp_min(0)
