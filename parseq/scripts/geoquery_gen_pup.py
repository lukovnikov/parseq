...
class LSTMPUPLoss(torch.nn.Module):
    def __init__(self, step=.20, minimum=.3, norm=1, **kw):
        super(LSTMPUPLoss, self).__init__(**kw)
        self.step = step
        self.minimum = minimum
        self.norm = norm
        self.eps = 1e-6

    def forward(self, states, refstates, levels):
        c = states.c
        refc = refstates.c.detach()
        diff = c - refc

        expected_sparsity = (levels.float() * self.step).clamp_max(1-self.minimum)
        measured_sparsities = (diff.abs() < self.eps).float().sum(-1) / (diff.size(-1))
        mask = measured_sparsities < expected_sparsity[:, None]

        diff_norms = torch.norm(diff, p=self.norm, dim=-1)

        ret = (diff_norms * mask.float()).sum(-1) / mask.size(-1)
        return ret
...
class BasicGenModel(TransitionModel):
    ...
    def forward(self, x:State):
        if not "mstate" in x:
            x.mstate = State()
            x.mstate.plosses = torch.zeros(len(x), device=x.inp_tensor.device)
            x.mstate.state_stack = np.array(range(len(x)), dtype="object")
            for i in range(len(x)):
                x.mstate.state_stack[i] = []
        mstate = x.mstate
        init_states = []
...
def run(lr=0.001,
    ...
    ploss = LSTMPUPLoss(step=p_step, minimum=p_min)
    ...
    losses = [CELoss(ignore_index=0, mode="logprobs"), StatePenalty("mstate.plosses", weight=beta)]
