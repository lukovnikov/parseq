...
def try_rr_lstm():
    t = StackLSTMTransition(5, 4, 2, dropout=0.1, p_step=0.3, p_min=0.3)
    x = torch.rand(3, 5)
...
class BasicGenModel(TransitionModel):
    ...
    def __init__(self, embdim, hdim, numlayers:int=1, dropout=0.,
        ...
        dec_rnn_in_dim = (encoder_dim if feedatt else 0)
        decoder_rnn = StackLSTMTransition(embdim, dec_rnn_in_dim, hdim, numlayers, dropout=dropout)
